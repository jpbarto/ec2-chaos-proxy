...
echo Chaos Gateway Security Group: $(aws cloudformation describe-stacks --stack-name chaos-gateway --query 'Stacks[].Outputs[?OutputKey==`GatewaySecurityGroup`].OutputValue' --output text)
