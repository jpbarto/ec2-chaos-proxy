...
ORIG_ROUTE_TABLE_ID=$(aws ec2 describe-tags --filters Name=resource-id,Values=$SUBNET Name=key,Values=rollback-route-table --query 'Tags[0].Value' --output text --region $REGION)
